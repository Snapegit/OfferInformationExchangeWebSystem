		...
		<div class="app-contain">
			<el-form class="userinfo_form" ref="userinfoFormRef" :model="form" label-width="$template2.back.add.form.base.labelWidth">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="从事行业" prop="xingyeleixing">
							<el-select 
								class="list_sel" 
								v-model="user.xingyeleixing" 
								placeholder="请选择从事行业"
								>
								<el-option v-for="item in yonghuxingyeleixingLists" :label="item" :value="item"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
...
<script setup>
	...
	const yonghuxingyeleixingLists = ref([])
	const init = () => {
		...
		context?.$http({
			...
			yonghuxingyeleixingLists.value = res.data.data
		})
	}
